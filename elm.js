...
Elm.Murol.make = function (_elm) {
   ...
   var _op = {};
   var renderMainMenu = function (elems) {
      return A2($Html.ul,
      _U.list([$Html$Attributes.id("mainMenu")]),
      A2($List.map,function (e) {    return A2($Html.span,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"]," ",e))]));},elems));
   };
   var pageHeader = A2($Html.header,
   _U.list([]),
   _U.list([A2($Html.img,_U.list([$Html$Attributes.src("/images/pageaccueil.jpg")]),_U.list([]))
           ,A2($Html.h1,_U.list([$Html$Attributes.id("pageTitle")]),_U.list([$Html.text("Murol")]))
           ,A2($Html.h2,_U.list([$Html$Attributes.id("subTitle")]),_U.list([$Html.text("La commune de Murol vous souhaite la bienvenue")]))]));
   var view = F2(function (adress,model) {
      return A2($Html.div,_U.list([$Html$Attributes.id("container")]),_U.list([pageHeader,renderMainMenu(function (_) {    return _.mainMenu;}(model))]));
   });
   var update = F2(function (action,model) {    return model;});
   var menuElems = _U.list(["agenda","tourisme","vie locale","vie economique","mairie","culture et loisirs","numeros d\'urgences","petites annonces"]);
   var initialModel = {mainMenu: menuElems};
