...
Elm.Test.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "Start") {
            return {ctor: "_Tuple2",_0: _U.update(model,{debug: "started"}),_1: $Effects.tick(Tick)};
         } else {
            return {ctor: "_Tuple2",_0: _U.update(model,{debug: "hasTicked"}),_1: $Effects.none};
         }
   });
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Start)]),_U.list([$Html.text("start")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(function (_) {    return _.debug;}(model))]))]));
   });
   var Model = function (a) {    return {debug: a};};
   var initialModel = Model("initial");
   ...
   return _elm.Test.values = {_op: _op,Model: Model,Start: Start,Tick: Tick,initialModel: initialModel,update: update,view: view,app: app,main: main};
};
