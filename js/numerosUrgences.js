...
Elm.NumerosUrgences.make = function (_elm) {
   ...
   var catToTable = function (cat) {
      ...
      function (_p2) {
         ...
         return A2($Html.tr,
         _U.list([]),
         _U.list([A2($Html.td,_U.list([]),_U.list([$Html.text(_p3._0)])),A2($Html.td,_U.list([]),_U.list([$Html.text(_p3._1)]))]));
      },
      es);
      var t = function (_) {    return _.title;}(cat);
      return A2($Html.table,_U.list([$Html$Attributes.id(t)]),A2($List._op["::"],A2($Html.th,_U.list([]),_U.list([$Html.text(t)])),rows));
   };
   ...
   _U.list([urgenceDiv,pratiqueDiv]));
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("container")]),
