...
Elm.SallesFetes.make = function (_elm) {
   ...
   var initialContent = {wrapper: function (content) {
                        ...
                        ,tiledMenu: $TiledMenu.init(_U.list([{ctor: "_Tuple3"
                                                             ...
                                                             _U.list([$Html$Attributes.$class("subContainerData noSubmenu")
                                                                     ,$Html$Attributes.id("calSalleFete")]),
                                                             _U.list([A2($Html.h4,_U.list([]),_U.list([$Html.text("Calendrier")]))
                                                                     ,A2($Html.div,
                                                                     _U.list([$Html$Attributes.id("bigAgenda")]),
                                                                     _U.list([A2($Html.iframe,
                                                                     _U.list([$Html$Attributes.src("https://calendar.google.com/calendar/embed?showTitle=0&height=600&wkst=1&amp;bgcolor=%23FFFFFF&src=r46rbonnui234n2b2glau5btoo%40group.calendar.google.com&color=%231B887A&amp;ctz=Europe%2FParis")]),
                                                                     _U.list([]))]))]))])}
                                                            ,{ctor: "_Tuple3",_0: "Salle des fêtes de Beaune",_1: "",_2: _U.list([])}]))};
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "NoOp") {
            return model;
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("container")]),
      _U.list([A2($Murol.renderMainMenu$,_U.list(["Mairie","Salles municipales"]),function (_) {    return _.mainMenu;}(model))
              ,A2($Html.div,
              _U.list([$Html$Attributes.id("subContainer")]),
